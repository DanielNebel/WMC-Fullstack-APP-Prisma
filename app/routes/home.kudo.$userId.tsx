...
import { validateColor, validateEmoji } from '~/utils/validators.server' 
...
export const action: ActionFunction = async ({ request }) => {
  ...
  // Validierung der Farben und Emojis
  ...
  // Konvertieren der IDs in Zahlen
      ...
      <form method="post">
        ...
        <div className="flex flex-col md:flex-row gap-y-2 md:gap-y-0">
          <div className="text-center flex flex-col items-center gap-y-2 pr-8">
            <UserCircle user={recipient} className="h-24 w-24" />
            <p className="text-blue-300">
              {recipient.firstName} {recipient.lastName}
            </p>
            {recipient.department && (
              <span className="px-2 py-1 bg-gray-300 rounded-xl text-blue-300 w-auto">
                {recipient.department.charAt(0).toUpperCase() +
                  recipient.department.slice(1).toLowerCase()}
              </span>
            )}
          </div>
